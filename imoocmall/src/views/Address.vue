<template>
  <div>
      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="next-btn-wrap">
            <router-link class="btn btn--m btn--red" :to="{path:'orderConfirm',query:{'addressId':selectedAddressId}}">
              Next</router-link>
          </div>
        </div>
      </div>
    ...
    <modal :mdShow="isMdshow" @close="closeModal">
      ...
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascrip:void(0);" @click="delAddress(addressId)">确认</a>
        <a class="btn btn--m" href="javascrip:void(0);" @click="isMdshow=false">取消</a>
      </div>
    </modal>
      ...
      init() {
        axios.get('/users/addressList').then((res) => {
          ...
          for (var i = 0; i <= res.data.result.length; i++) {
            if (res.data.result[i].isDefault == true) {
              this.checkIndex = i
              this.selectedAddressId = res.data.result[i].addressId
            }
          }
        })
      },
